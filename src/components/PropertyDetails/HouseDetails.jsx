  ...
  return (
    <>
      <Stack direction={{base: 'column', md: 'row'}} justify='space-between' align={{md: 'center'}}  my='28px'>
        ...
        <HStack>
          <Text px='3' borderRadius='full' bg='green.300'>{searchedHouse.developer}</Text>
          <Text px='3' borderRadius='full' bg='purple.300'>{searchedHouse.Area}</Text>
        </HStack>

        <Text fontWeight="extrabold" fontSize="20px" color="blue.500">Rs. {(searchedHouse.price<100? searchedHouse.price : (searchedHouse.price/100))}  <span style={{ fontSize: 12, color: "grey", fontWeight: "normal" }}>
            {(searchedHouse.price<100? "lacs" : "Cr")}
            </span></Text>
